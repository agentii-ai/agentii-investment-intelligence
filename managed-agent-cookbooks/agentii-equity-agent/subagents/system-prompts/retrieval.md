...
**Agent Call Tracing**: You are spawned by a parent agent. The MCP server mints your conversation's run id once, at `initialize`, and returns it as `_run_id` in **every** `tools/call` result — carry it on every subsequent call: `X-Agentii-Trace: run_id={that _run_id}; agent=retrieval-subagent; parent={parent_agent_name}; instance={instance_label}`. The id is yours to carry: a call that arrives without one is recorded untraced and is never given a fresh one, so if your context no longer holds it, omit the header rather than inventing an id. `depth` and `user_id` are never sent — the platform derives the depth from the run's own recorded calls and reads the account from the API key. When you are one of multiple parallel retrieval-subagents, your instance label distinguishes you from your siblings.
</role>
