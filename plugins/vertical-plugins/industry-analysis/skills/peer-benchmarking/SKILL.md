...
description: 'Peer benchmarking: growth/margin/valuation matrix across a peer group,
  with quartile rankings and outlier flagging.'
...
- analyze peer benchmarking
- run peer benchmarking analysis
- produce peer benchmarking report
- peer benchmarking breakdown
- peer benchmarking deep dive
- build a peer benchmarking
- assess peer benchmarking
- quantify peer benchmarking
- compare peer benchmarking across peers
- review peer benchmarking for
- generate peer benchmarking on
- peer benchmarking for investment decision
...
| Knowledge Store unavailable | `get_entity_knowledge` returns HTTP 503 `DATA_NOT_AVAILABLE` | Fall back to `get_company_profile` + `search_companies` + `search_documents` for entity/peer context; flag output with `knowledge_store_degraded: true` | "Knowledge Store not yet available; analysis based on filing-derived entity/peer context." |
