...
description: Sector overview, TAM estimation, competitive concentration HHI, regulatory landscape, industry analysis, market size analysis, sector trends, industry structure, market growth rate, sector profitability
temporal_scope:
 default_quarters: 4
 max_quarters: 12
...
- `search_unified` — used by this skill per the retrieval strategy
...
Write to `{ticker}/{YYYY-MM-DD_HHMM}_sector-overview_{affix}.md` (see `## Output File`).
