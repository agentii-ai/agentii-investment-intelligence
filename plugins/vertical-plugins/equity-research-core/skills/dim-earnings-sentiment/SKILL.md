...
temporal_scope:
  default_quarters: 4
  max_quarters: 8
  description: 'Earnings sentiment: trailing 4 quarters for earnings-call tone and
    guidance trends'
allowed_tools:
- search_xbrl_facts
- list_xbrl_concepts
...
essentials_modes:
- analyst-sentiment-assessment-current-quarter
- fy0-analyst-estimates-extraction
...
Analyze sell-side preview reports to assess analyst sentiment and conviction level
regarding the company's expected financial performance for the upcoming earnings release.

Classify analyst tone for each key financial metric as:
- Highly Convicted: Strong confidence actual results will EXCEED estimates
- Positive (In Line): Results expected to MEET or ALIGN CLOSELY with estimates
- Negative (Below): Concern that actual results may FALL SHORT of estimates
- N/A: No estimates provided for the metric
...
Extract and calculate mean (average) analyst estimates for the current fiscal year (FY0)
from Morgan Stanley and Jefferies preview reports for upcoming earnings release.
...
- **structure**: ## Current Fiscal Year (FY0) Analyst Estimates
...
Extract and calculate mean analyst estimates from Morgan Stanley, Jefferies, and SEC filings
for BOTH:
- Current fiscal quarter (to be reported on upcoming earnings date)
- Current fiscal year (FY0)
...
- **structure**: ## Analyst Estimates – Current Fiscal Quarter & Fiscal Year
...
Extract management's official guidance for current quarter and fiscal year
from last quarterly earnings disclosures (8-K filings, earnings presentations).

Use midpoint if range provided. Average across documents if multiple sources.
...
- **structure**: ## Management Guidance – Current Quarter & Fiscal Year
...
- **structure**: ## Current-Quarter Estimates vs. Management Guidance
...
- **structure**: ## Full-Year Analyst Estimates vs. Management Guidance
