...
**Layer 2 page-relevance signal**: Score pages using the `description` (a **platform-generated** summary — median **204 characters** over 47,980 LLM rows, `labels_census.py`, 2026-09-23; this line said "~100-char", an understatement of ~2.0×) AND the `keywords` (extracted entity terms). Both fields are populated on **95.7%** of `pipeline.src_silver_pages` rows (2,907,053 of 3,036,552, measured 2026-09-23; this line said "96%+", which was an unmeasured assertion). **`description` is not the filing's text**: quote pages, not descriptions — the served record's `description_provenance` says which kind of generated string it is. For business-model analysis, prefer pages whose `keywords` contain entity terms like product names, segment names, geographies, channel partners, executive names.
...
5. **Layer 3 Deep Read**: `read_source_pages/{ticker}/{citation_id}?pages=page<N1>,page<N2>` — load full `page_content` for ONLY the 3-5 pages identified in Layer 2 to enrich the XBRL backbone with narrative.
...
The Neon production database and `api.agentii.ai` REST/MCP surfaces are LIVE and AUTHORITATIVE as of 2026-05-25. All retrieval planning MUST treat these as ground truth. Production scale, **measured 2026-09-23**: 3,036,552 `pipeline.src_silver_pages` over 95,857 `pipeline.src_documents` (this line said 1.34M pages / 51,089 documents — superseded snapshots), covering all 5 SEC form types (8-K/10-K/10-Q/6-K/20-F) plus earnings-call transcripts; `labels` JSONB with `general.description` + `general.keywords`; `secondary_labels` GIN-indexed. 75,967 `pipeline.earnings_calendar` rows. Always call `get_ticker_coverage/{ticker}` before retrieval planning.
