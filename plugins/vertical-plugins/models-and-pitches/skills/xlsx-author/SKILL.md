---
name: xlsx-author
description: 'Excel workbook author: build formula-driven workbooks from an xlsx_spec
  using openpyxl-headless or Office JS binding.'
...
- analyze xlsx author
- run xlsx author analysis
- produce xlsx author report
- xlsx author breakdown
- xlsx author deep dive
- build a xlsx author
- assess xlsx author
- quantify xlsx author
- compare xlsx author across peers
- review xlsx author for
- generate xlsx author on
- xlsx author for investment decision
...
```
[custom Python script] → xlsx_build(method: script) → xlsx_recalc → xlsx_audit → [.xlsx output]
```

## Validation Gates

1. **output integrity**: .xlsx opens without corruption. *If failed*: If corrupted: refuse delivery.
2. **named ranges**: all named ranges resolve. *If failed*: If unresolved: refuse delivery.
3. **script execution**: Python script executed without exception. *If failed*: If fails: return stderr.

4. **tool diversity**: distinct MCP tools used in this invocation >= `min_tool_diversity` (3). *If failed*: flag as depth-insufficient in Coverage Gaps, listing which tool categories were unused (structured data / document retrieval / company metadata / earnings calendar / coverage). This gate does NOT block analysis completion — it is a quality signal for your review.
