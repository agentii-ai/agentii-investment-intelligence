...
**Ticker resolution (FR-082)** and **Workspace style.md override check (FR-094)** apply. **`get_realtime_quote` availability (FR-105)**: If not deployed, prompt user for current price and dividend yield. Beta from industry average (Damodaran) as fallback.
...
Write to `{ticker}/{YYYY-MM-DD_HHMM}_ddm-valuation_dividend-model.md` per FR-079.
